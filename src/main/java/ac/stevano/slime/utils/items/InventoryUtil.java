...
public class InventoryUtil {
    ...
    public int freeSlots(Inventory inventory) {
        int i = 0;
    ...
    public boolean hasItem(Inventory inventory, ItemStack itemStack) {
        if (freeSpace(inventory)) return false;
        for (ItemStack item : inventory.getContents()) if (item.equals(itemStack)) return true;
        return false;
    }
}